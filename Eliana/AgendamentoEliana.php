...
if ($logado == 'suplili@gmail.com') {
    if (!empty($_GET['search'])) {
        $data = $_GET['search'];
        $sql = "SELECT * FROM agendamento_lili 
        WHERE (id_agendamentolili LIKE '%$data%' OR nome LIKE '%$data%' OR email LIKE '%$data%') 
        ORDER BY id_agendamentolili DESC";
    } else {
        $sql = "SELECT * FROM agendamento_lili ORDER BY id_agendamentolili DESC";
    }
} else {
    if (!empty($_GET['search'])) {
        $data = $_GET['search'];
        $sql = "SELECT * FROM agendamento_lili 
        WHERE email = '$logado' 
        AND (id_agendamentolili LIKE '%$data%' OR nome LIKE '%$data%' OR email LIKE '%$data%') 
        ORDER BY id_agendamentolili DESC";
    } else {
        $sql = "SELECT * FROM agendamento_lili WHERE email = '$logado' ORDER BY id_agendamentolili DESC";
    }
}

$result = $conexao->query($sql);

$sql_retroativo = "UPDATE agendamento_lili 
...
<html lang="pt-br">
  <head>
    ...
    <title>Agendamento | Eliana</title>
  </head>
  <body>
    ...
    <hr />
    <main>
    <p style="text-align: center; font-size: 18px; padding-bottom: 15px;">Eliana Agendamentos:</p>
  ...
  <tbody>
    <?php
      ...
      {
        echo "<tr>";
        echo "<td style='text-align: center;'>" . $user_data['id_agendamentolili'] . "</td>";
        ...
        echo "<td style='text-align: center;'>" . $user_data['email'] . "</td>";
          ...
          echo "<td>
                    <a class='btn btn-sm lapis' href='editEliana.php?id=$user_data[id_agendamentolili]'>
                    ...
                    <a class='btn btn-sm btn-danger' href='deleteEliana.php?id=$user_data[id_agendamentolili]'>
  ...
  <script>
    ...
    function searchData(){
      window.location = 'AgendamentoEliana.php?search=' + search.value;
    }
  </script>
</html>
