...
if ($logado == 'sup@gmail.com') {
    ...
    if (!empty($_GET['search'])) {
        $data = $_GET['search'];
        $sql = "SELECT * FROM agendamentos_natalia 
        WHERE (id_agendamentoNatalia LIKE '%$data%' OR nome LIKE '%$data%' OR email LIKE '%$data%') 
        ORDER BY id_agendamentoNatalia DESC";
    } else {
        $sql = "SELECT * FROM agendamentos_natalia ORDER BY id_agendamentoNatalia DESC";
    }
} else {
    if (!empty($_GET['search'])) {
        $data = $_GET['search'];
        $sql = "SELECT * FROM agendamentos_natalia 
        WHERE email = '$logado' 
        AND (id_agendamentoNatalia LIKE '%$data%' OR nome LIKE '%$data%' OR email LIKE '%$data%') 
        ORDER BY id_agendamentoNatalia DESC";
    } else {
        $sql = "SELECT * FROM agendamentos_natalia WHERE email = '$logado' ORDER BY id_agendamentoNatalia DESC";
    }
}

$result = $conexao->query($sql);

$sql_retroativo = "UPDATE agendamentos_natalia 
...
<html lang="pt-br">
  <head>
    ...
    <title>Agendamento | Natalia</title>
  </head>
  <body>
    <header id="home-header" class="cabecalho">
      ...
      <div class="btns">
        <div class="btn-agendar">
          <a href="../Natalia/NataliaSilva.php">Agendar</a>
        </div>

        <div class="btn-agendar">
          <a href="../Natalia/NataliaSilva.php">Voltar</a>
        </div>
      </div>
    </header>
    <hr />
    <main>
    <p style="text-align: center; font-size: 18px; padding-bottom: 15px;">Natalia Agendamentos:</p>
  ...
  <tbody>
    <?php
      ...
      {
        echo "<tr>";
        echo "<td style='text-align: center;'>" . $user_data['id_agendamentoNatalia'] . "</td>";
        ...
        echo "<td style='text-align: center;'>" . $user_data['email'] . "</td>";
          ...
          echo "<td style='text-align: center;'>" . $user_data['recado'] . "</td>";
          echo "<td>
                    <a class='btn btn-sm lapis' href='editNatalia.php?id=$user_data[id_agendamentoNatalia]'>
                    ...
                    <a class='btn btn-sm btn-danger' href='deleteNatalia.php?id=$user_data[id_agendamentoNatalia]'>
  ...
  <script>
    ...
    function searchData(){
      window.location = 'AgendamentoNatalia.php?search=' + search.value;
    }
  </script>
</html>
